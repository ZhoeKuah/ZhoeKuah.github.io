...
// --- STYLING HELPERS ---
const swotStyles = {
  emerald: { border: 'border-emerald-500/30', hover: 'hover:border-emerald-500/60', text: 'text-emerald-400', bullet: 'text-emerald-500', icon: 'text-emerald-400' },
  red: { border: 'border-red-500/30', hover: 'hover:border-red-500/60', text: 'text-red-400', bullet: 'text-red-500', icon: 'text-red-400' },
  blue: { border: 'border-blue-500/30', hover: 'hover:border-blue-500/60', text: 'text-blue-400', bullet: 'text-blue-500', icon: 'text-blue-400' },
  yellow: { border: 'border-yellow-500/30', hover: 'hover:border-yellow-500/60', text: 'text-yellow-400', bullet: 'text-yellow-500', icon: 'text-yellow-400' },
};
...
const LanguageRing = ({ language, level, color, delay }: { language: string, level: number, color: string, delay: number }) => {
  ...
  const getColor = (c: string) => {
    const colors: Record<string, string> = { cyan: '#22d3ee', blue: '#3b82f6', purple: '#a855f7', pink: '#ec4899', emerald: '#10b981', red: '#ef4444', green: '#22c55e', orange: '#f97316' };
    return colors[c] || '#fff';
  };
  return (
    <div className="flex flex-col items-center gap-2">
      <div className="relative w-24 h-24 flex items-center justify-center">
        <svg className="w-full h-full transform -rotate-90">
          ...
          <motion.circle cx="48" cy="48" r={radius} stroke={getColor(color)} strokeWidth="6" fill="transparent" strokeDasharray={circumference} initial={{ strokeDashoffset: circumference }} whileInView={{ strokeDashoffset }} viewport={{ once: true }} transition={{ duration: 1.5, delay, ease: "easeOut" }} strokeLinecap="round" />
        </svg>
...
const SwotCard = ({ title, items, icon: Icon, color }: { title: string, items: string[], icon: any, color: keyof typeof swotStyles }) => {
  const style = swotStyles[color];
  return (
    <motion.div whileHover={{ y: -5 }} className={`h-full flex flex-col bg-gray-900/30 border ${style.border} p-6 rounded-xl ${style.hover} transition-all duration-300`}>
      ...
      <section className="py-20 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto">
          <motion.div className="text-center mb-16" initial={{ opacity:0, y:20 }} animate={{ opacity:1, y:0 }}>
            <h1 className="text-5xl font-bold mb-4 bg-gradient-to-r from-emerald-400 via-teal-400 to-cyan-400 bg-clip-text text-transparent">The Human Behind the Code</h1>
          ...
          <div className="grid lg:grid-cols-3 gap-8 mb-20 items-stretch">
            ...
            >
              ...
              <p className="text-gray-300 text-lg leading-relaxed mb-6">I believe technical documentation should be human and accessible, bridging the gap between academic standards and real-world system challenges.</p>
